function trackedVid = run_particle(video,particle_n, patch,sigma, name)
...
k =1;
rng(0,'twister');
particle_y = randi([1 vidHeight],1,particle_n)';
particle_x = randi([1 vidWidth],1,particle_n)';
...
while hasFrame(video)
   ...
   S= particle_filter(S,mov(k).cdata,sigma,patch);
   [bestPart,I] = max(S(:,3));
   ypart = S(I,1);
   xpart= S(I,2);
