...
def reduce(image):
  ...
  conv = np.zeros([yShape, xShape ])
  
  conv[:,:] = scipy.signal.convolve2d(image[:,:], generatingKernel(.4), 'same')
    
  reducedImage = conv[:(conv.shape[0]):2, :(conv.shape[1]):2]
...
def expand(image):
  ...
  conv = np.zeros([yShape*2, xShape*2])
  
  conv[:conv.shape[0]:2, :conv.shape[1]:2] = image
  
  
  conv[:,:] = scipy.signal.convolve2d(conv[:,:], generatingKernel(.4), 'same')
  conv = conv * 4
...
def laplPyramid(gaussPyr):
  ...
  for layer in range(sizeOfPyr):
    ...
    yShape, xShape = gaussPyr[layer].shape
    expanded = expand(gaussPyr[layer+1])[:yShape,:xShape]
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
  ...
  for layer in range(sizeOfPyr):
    ...
    blended_pyr.append(gaussPyrMask[layer] * laplPyrWhite[layer] + (1 - gaussPyrMask[layer]) * laplPyrBlack[layer])
...
def collapse(pyramid):
  ...
  for layer in range(sizeOfPyr-1,-1,-1):
    if layer == 0:
      ...
      break
    yShape, xShape = pyramid[layer-1].shape
    expanded = expand(pyramid[layer])[:yShape,:xShape]
