...
def findHomography(image_1_kp, image_2_kp, matches):
    ...
    image_1_points = np.zeros((len(matches), 1, 2), dtype=np.float64)
    image_2_points = np.zeros((len(matches), 1, 2), dtype=np.float64)
    i = 0
...
def warpImagePair(image_1, image_2, homography):
    ...
    all_corners = np.zeros([8,1,2])
    
    # WRITE YOUR CODE HERE
    corn1 = cv2.perspectiveTransform(image_1_corners,homography)
    all_corners[:4] = corn1
    all_corners[4:] = getImageCorners(image_2)
    

    x_min = np.int16(np.min(all_corners[:,:,0]))
    x_max = np.int16(np.max(all_corners[:,:,0]))
    y_min = np.int16(np.min(all_corners[:,:,1]))
    y_max = np.int16(np.max(all_corners[:,:,1]))
