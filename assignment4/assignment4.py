...
def imageGradientX(image):
    ...
    shift1 = np.float64(np.roll(image,-1,axis=1))
...
def computeGradient(image, kernel):
    ...
    rows = rows -1
    columns = columns - 1
    new_image = np.zeros((rows,columns), np.float64)
    for row in range(1,rows):
        for column in range(1,columns):
            F = image[row-1:row+2,column-1:column+2]
            
            summed = np.tensordot(F, kernel, axes=2)
            value = summed/8
            new_image[row,column] = value
    return new_image
